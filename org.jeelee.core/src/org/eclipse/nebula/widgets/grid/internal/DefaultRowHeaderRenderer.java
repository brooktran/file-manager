...
public class DefaultRowHeaderRenderer extends AbstractRenderer {
	...
	public DefaultRowHeaderRenderer(Grid grid) {
		super(grid);
	}
	...
	public void paint(GC gc, Object value) {
		...
		if (isSelected() && item.getParent().getCellSelectionEnabled()) {
			gc.setBackground(item.getParent()
		...
		if (image != null) {
			if (isSelected() && !item.getParent().getCellSelectionEnabled()) {
				gc.drawImage(image, x + 1, getBounds().y + 1
						+ (getBounds().height - image.getBounds().height) / 2);
				x += 1;
			} else {
				gc.drawImage(image, x, getBounds().y
						+ (getBounds().height - image.getBounds().height) / 2);
			}
		...
		if (foreground == null) {
			foreground = getDisplay().getSystemColor(
		...
		if (!item.getParent().isWordWrapHeader()) {
			...
			gc.drawString(TextUtils.getShortString(gc, text, width),
					getBounds().x + x + selectionOffset, y + selectionOffset,
					true);
		} else {
			...
			if (item.getParent().isAutoHeight()) {
				// Look through all columns to get the max height needed for
				// this item
				int columnCount = item.getParent().getColumnCount();
				int maxHeight = textLayout.getBounds().height + topMargin
						+ bottomMargin;
			...
			textLayout.draw(gc, getBounds().x + x + selectionOffset, y
					+ selectionOffset);
		}
	...
	public Point computeSize(GC gc, int wHint, int hHint, Object value) {
		...
		if (image != null) {
			y += Math.max(gc.getFontMetrics().getHeight(),
