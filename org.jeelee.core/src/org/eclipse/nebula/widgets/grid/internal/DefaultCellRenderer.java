...
public class DefaultCellRenderer extends GridCellRenderer {
	...
	public DefaultCellRenderer(Grid grid) {
		super(grid);
	}
	...
	public void paint(GC gc,Object value) {
		...
		if (drawAsSelected) {
			...
			if (item.getParent().isEnabled()) {
				Color back = getItemBackground(item);
		...
		if (drawBackground) {
			gc.fillRectangle(getBounds().x, getBounds().y, 
					getBounds().width,
					getBounds().height);
		}
		...
		if (!isWordWrap()) {
			String text = TextUtils.getShortString(gc,
					item.getText(getColumn()), width);
			...
			if (item.getParent().isAutoHeight()) {
				...
				int maxHeight = textLayout.getBounds().height + textTopMargin
						+ textBottomMargin;
				for (int i = 0; i < columnCount; i++) {
					GridColumn column = item.getParent().getColumn(i);
					if (i != getColumn() && column.getWordWrap()) {
						int height = column.getCellRenderer().computeSize(gc,
								column.getWidth(), SWT.DEFAULT, item).y;
				...
				if (item.getParent().isWordWrapHeader()) {
					int height = item.getParent().getRowHeaderRenderer()
							.computeSize(gc, SWT.DEFAULT, SWT.DEFAULT, item).y;
					maxHeight = Math.max(maxHeight, height);
				}
		...
		if (isCellFocus()) {
			Rectangle focusRect = new Rectangle(getBounds().x, getBounds().y,
					getBounds().width - 1, getBounds().height);
	...
	public void setTree(boolean tree) {
		...
		if (tree) {
			toggleRenderer = new ToggleRenderer(grid);
			toggleRenderer.setDisplay(getDisplay());

			branchRenderer = new BranchRenderer(grid);
	...
	public void setCheck(boolean check) {
		...
		if (check) {
			checkRenderer = new CheckBoxRenderer(grid);
