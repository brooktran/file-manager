...
import org.eclipse.nebula.widgets.grid.internal.IRenderer;
 ...
 * @author c.zhiwu@gmail.com
 */
public class Grid extends Canvas {
	// TODO: figure out better way to allow renderers to trigger events
	// TODO: scroll as necessary when performing drag select (current strategy
	// ok)
	// TODO: need to refactor the way the range select remembers older selection
	// TODO: remember why i decided i needed to refactor the way the range
	// select remembers older selection
	 ...
	 */
	protected boolean scrollValuesObsolete = false;
	 ...
	 * All root items.
	 */
	final List<GridItem> rootItems = new ArrayList<GridItem>();
	 ...
	 */
	private GridItem focusItem;// MMMM

	private boolean cellSelectionEnabled = false;

	final List<Point> selectedCells = new ArrayList<Point>();
	...
	final List<GridColumn> selectedColumns = new ArrayList<GridColumn>();

	/**
	 * This is the column that the user last navigated to, but may not be the
	 * focusColumn because that column may be spanned in the current row. This
	 * is only used in situations where the user has used the keyboard to
	 * navigate up or down in the table and the focusColumn has switched to a
	 * new column because the intended column (was maintained in this var) was
	 * spanned. The table will attempt to set focus back to the intended column
	 * during subsequent up/down navigations.
	 */
	...
	/**
	 * Renderer to paint the bottom left area when row headers and column
	 * footers are shown
	 */
	...
	/**
	 * Default height of items. This value is used for <code>GridItem</code>s
	 * with a height of -1.
	 */
...
//	private GridColumn columnBeingSorted;
	...
	/**
	 * Dispose listener. This listener is removed during the dispose event to
	 * allow re-firing of the event.
	 */
	 ...
	 * and various places for optimization.
	 */
	...
	/**
	 * True if three is at least one cell spanning columns. This is used in
	 * various places for optimizatoin.
	 */
	private boolean hasSpanning = false;

	/**
	 * Index of first visible item. The value must never be read directly. It is
	 * cached and updated when appropriate. #getTopIndex should be called for
	 * every client (even internal callers). A value of -1 indicates that the
	 * value is old and will be recomputed.
	 * 
	 * @see #bottomIndex
	 */
	int topIndex = -1;
	 ...
	 * cached and updated when appropriate. #getBottomIndex() should be called
	 * for every client (even internal callers). A value of -1 indicates that
	 * the value is old and will be recomputed.
	 * <p>
	 * Note that the item with this index is often only partly visible; maybe
	 * only a single line of pixels is visible. In extreme cases, bottomIndex
	 * may be the same as topIndex.
	 * 
	 * @see #topIndex
	 */
	int bottomIndex = -1;
	...
	/**
	 * Index of the first visible column. A value of -1 indicates that the value
	 * is old and will be recomputed.
	 */
	int startColumnIndex = -1;

	/**
	 * Index of the the last visible column. A value of -1 indicates that the
	 * value is old and will be recomputed.
	 */
	int endColumnIndex = -1;

	/**
	 * True if the last visible item is completely visible. The value must never
	 * be read directly. It is cached and updated when appropriate. #isShown()
	 * should be called for every client (even internal callers).
	 * 
	 * @see #bottomIndex
	 */
	boolean bottomIndexShownCompletely = false;
	 ...
	 * This is used to mimic Table behavior that resizes the rows on the first
	 * image added. See imageSetOnItem.
	 */
	 ...
	 * setCapture(true). The OS (SWT?) will automatically capture the mouse for
	 * us during a drag operation.
	 */
	private boolean inplaceTooltipCapture;

	/**
	 * This is the tooltip text currently used. This could be the tooltip text
	 * for the currently hovered cell, or the general grid tooltip. See
	 * handleCellHover.
	 */
	 ...
	 * {@link GridItem#isVisible()} == true). Therefore, the items at
	 * 'startIndex' and 'endIndex' are always visible.
	 * 
	...
	private static class RowRange {
		...
		public int rows;
		/**
		 * height in pixels of this range (including horizontal separator
		 * between rows)
		 */
		public int height;
	}
	...
	public Grid(Composite parent, int style) {
		...
		focusRenderer = new DefaultFocusRenderer(this);
		...
		itemHeight = sizingGC.getFontMetrics().getHeight() + 3;// 2

		RGB sel = getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION)
				.getRGB();
	...
	/**
	 * Returns the background color of column and row headers when a cell in the
	 * row or header is selected.
	 * 
	...
	/**
	 * Sets the background color of column and row headers displayed when a cell
	 * in the row or header is selected.
	 * 
	 ...
	 * <p>
	 * Cell selection events may have <code>Event.detail = SWT.DRAG</code> when
	 * the user is drag selecting multiple cells. A follow up selection event
	 * will be generated when the drag is complete.
	 * 
	 * @param listener
	 ...
	 * <p>
	 * If cell selection is enabled, all cells in the specified item are
	 * deselected.
	 * 
	 * @param index
	 ...
	 * <p>
	 * If cell selection is enabled, all cells in the given range are
	 * deselected.
	 * 
	 * @param start
	 ...
	 * ignored.
	 * <p>
	 * If cell selection is enabled, all cells in the given items are
	 * deselected.
	 * 
	 * @param indices
	...
	/**
	 * Deselects all selected items in the receiver. If cell selection is
	 * enabled, all cells are deselected.
	 * 
	 * @throws org.eclipse.swt.SWTException
	 *             <ul>
	 *             <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed
	 ...
	 * null if no such column exists. The point is in the coordinate system of
	 * the receiver.
	 * 
	 * @param item
	 *            a known GridItem
	...
	private GridColumn getColumn(GridItem item, Point point) {
		...
		for (Iterator<GridColumn> columnIterator = displayOrderedColumns.iterator(); columnIterator
				.hasNext();) {
			GridColumn column = columnIterator.next();
	 ...
	 * occurs when the programmer uses the table like a list, adding items but
	...
	public int[] getColumnOrder() {
		...
		int i = 0;
		for (Iterator colIterator = displayOrderedColumns.iterator(); colIterator
				.hasNext();) {
	...
	/**
	 * Returns an array of {@code GridColumnGroup}s which are the column groups
	 * in the receiver.
	 * <p>
	 * Note: This is not the actual structure used by the receiver to maintain
	 * its list of items, so modifying the array will not affect the receiver.
	...
	/**
	 * Returns the column group at the given, zero-relative index in the
	 ...
	 * receiver. If no {@code GridColumn}s were created by the programmer, the
	 * array is empty, despite the fact that visually, one column of items may
	 * be visible. This occurs when the programmer uses the table like a list,
	...
	/**
	 * Returns {@code true} if the receiver's footer is visible, and
	 * {@code false} otherwise
	 * 
	...
	public GridItem getItem(Point point) {
		...
		int row = getTopIndex();
		while (row <getItemCount() && y2 <= getClientArea().height) {
			GridItem currItem = getItem(row);
...
//		return getItems().length;
	...
	private void addItem(int index, GridItem item) {
	...
	/**
	 * Sets the default height for this <code>Grid</code>'s items. When this
	 * method is called, all existing items are resized to the specified height
	 * and items created afterwards will be initially sized to this height.
	 * <p>
	 * As long as no default height was set by the client through this method,
	 * the preferred height of the first item in this <code>Grid</code> is used
	 * as a default for all items (and is returned by {@link #getItemHeight()}).
	 * 
	 * @param height
	...
	/**
	 * Returns a (possibly empty) array of {@code GridItem}s which are the items
	 * in the receiver.
	 * <p>
	 * Note: This is not the actual structure used by the receiver to maintain
	 * its list of items, so modifying the array will not affect the receiver.
	...
	public GridItem[] getItems() {
		checkWidget();
		return items.toArray(new GridItem[getItemCount()]);
	}
	...
	public GridItem getNextVisibleItem(GridItem item) {
		...
		GridItem nextItem = getItem(index + 1);
	...
	public GridItem getPreviousVisibleItem(GridItem item) {
		...
		GridItem prevItem = getItem(index - 1);
	...
	public GridItem[] getRootItems() {
		...
		return rootItems.toArray(new GridItem[getRootItemCount()]);
	}
	...
	public GridItem getRootItem(int index) {
		...
		if (index < 0 || index >= getRootItemCount()) {
			SWT.error(SWT.ERROR_INVALID_RANGE);
		}
	 ...
	 * <p>
	 * If cell selection is enabled, any items which contain at least one
	 * selected cell are returned.
	 * 
	 ...
	 * selection is enabled, the number of items with at least one selected cell
	 * are returned.
	 * 
	 ...
	 * in the receiver, or -1 if no item is selected. If cell selection is
	 * enabled, returns the index of first item that contains at least one
	 * selected cell.
	 * 
	...
	public int getSelectionIndex() {
		...
		if (!cellSelectionEnabled) {
			...
			return getIndexOfItem(selectedItems.get(0));
		} else {
	...
	public int[] getSelectionIndices() {
		...
		if (!cellSelectionEnabled) {
			...
			int i = 0;
			for (Iterator itemIterator = selectedItems.iterator(); itemIterator
					.hasNext();) {
				GridItem item = (GridItem) itemIterator.next();
				indices[i] = getIndexOfItem(item);
				i++;
			}
			return indices;
		} else {
			...
			int i = 0;
			for (Iterator itemIterator = selectedRows.iterator(); itemIterator
					.hasNext();) {
				GridItem item = (GridItem) itemIterator.next();
				indices[i] = getIndexOfItem(item);
				i++;
	...
	public int getTopIndex() {
		...
		if (!vScroll.getVisible()) {
			...
			if (isTree) {
				Iterator itemsIter = itemIterator();
				...
				while (row > 0 && itemsIter.hasNext()) {
					...
					if (item.isVisible()) {
						row--;
						if (row == 0) {
							firstVisibleIndex = getIndexOfItem(item);
						}
					}
				}
			}
			...
			/*
			 * MOPR here lies more potential for increasing performance for the
			 * case (isTree || hasDifferingHeights) the topIndex could be
			 * derived from the previous value depending on a delta of the
			 * vScroll.getSelection() instead of being calculated completely
			 * anew
			 */
		}
	...
	/**
	 * Returns the zero-relative index of the item which is currently at the
	 * bottom of the receiver. This index can change when items are scrolled,
	 * expanded or collapsed or new items are added or removed.
	 * <p>
	 * Note that the item with this index is often only partly visible; maybe
	 * only a single line of pixels is visible. Use {@link #isShown(GridItem)}
	 * to find out.
	 * <p>
	...
	int getBottomIndex() {
		...
		if (getItemCount() == 0) {
			...
			RowRange range = getRowRange(getTopIndex(), getVisibleGridHeight(),
					false, false);
	...
	/**
	 * Returns a {@link RowRange} ranging from the grid item at startIndex to
	 * that at endIndex.
	 * <p>
	 * This is primarily used to measure the height in pixel of such a range and
	 * to count the number of visible grid items within the range.
	 * 
	 * @param startIndex
	 *            index of the first item in the range or -1 to the first
	 *            visible item in this grid
	 * @param endIndex
	 *            index of the last item in the range or -1 to use the last
	 *            visible item in this grid
	 * @return
	 */
	private RowRange getRowRange(int startIndex, int endIndex) {
		...
		if (startIndex == -1) {
			...
			do {
				startIndex++;
			} while (startIndex < getItemCount()
					&& !getItem(startIndex).isVisible());
		...
		if (endIndex == -1) {
			...
			do {
				endIndex--;
			} while (endIndex >= 0 && !getItem(endIndex).isVisible());
			if (endIndex == -1) {
				return null;
			}
		}
		...
		// fail fast
		if (startIndex < 0 || endIndex < 0 || startIndex >= getItemCount()
				|| endIndex >= getItemCount() || endIndex < startIndex
				|| getItem(startIndex).isVisible() == false
				|| getItem(endIndex).isVisible() == false) {
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		}
	...
	/**
	 * This method can be used to build a range of grid rows that is allowed to
	 * span a certain height in pixels.
	 * <p>
	 * It returns a {@link RowRange} that contains information about the range,
	 * especially the index of the last element in the range (or if inverse ==
	 * true, then the index of the first element).
	 * <p>
	 * Note: Even if 'forceEndCompletelyInside' is set to true, the last item
	 * will not lie completely within the availableHeight, if (height of item at
	 * startIndex < availableHeight).
	 * 
	 * @param startIndex
	 *            index of the first (if inverse==false) or last (if
	 *            inverse==true) item in the range
	 * @param availableHeight
	 *            height in pixels
	 * @param forceEndCompletelyInside
	 *            if true, the last item in the range will lie completely within
	 *            the availableHeight, otherwise it may lie partly outside this
	 *            range
	 * @param inverse
	 *            if true, then the first item in the range will be searched,
	 *            not the last
	...
	private RowRange getRowRange(int startIndex, int availableHeight,
		...
		if (startIndex == -1) {
			if (!inverse) {
				...
				do {
					startIndex--;
				} while (startIndex >= 0 && !getItem(startIndex).isVisible());
				if (startIndex == -1) {
					return null;
				}
			}
		...
		if (isTree || hasDifferingHeights) {
			...
			consumedItems++;
			consumedHeight += getItem(otherIndex).getHeight();

			// note: we use "+2" in next line, because we only try to add
			// another row if there
			// is room for the separator line + at least one pixel row for the
			// additional item
			while (consumedHeight + 2 <= availableHeight) {
				...
				do {
					...
					if (nextIndex >= 0 && nextIndex < getItemCount()) {
						nextItem = getItem(nextIndex);
					} else {
						nextItem = null;
					}
			...
			if (((getItemHeight() + 1) * range.rows - 1) + 1 < availableHeight) {
				...
				// - so add another row if it need not be completely within
				// availableHeight
				if (!forceEndCompletelyInside) {
					availableRows++;
				}
			...
			int otherIndex = startIndex
					+ ((availableRows - 1) * (!inverse ? 1 : -1));
			if (otherIndex < 0) {
				otherIndex = 0;
			}
			if (otherIndex >= getItemCount()) {
				otherIndex = getItemCount() - 1;
			}
	 ...
	 * {@link GridItem#isVisible()} ; not only those currently visible on
	...
	int getGridHeight() {
		...
		/*
		 * MOPR currently this method is only used in #getTableSize() ; if it
		 * will be used for more important things in the future (e.g. the max
		 * value for vScroll.setValues() when doing pixel-by-pixel vertical
		 * scrolling) then this value should at least be cached or even updated
		 * incrementally when grid items are added/removed or expaned/collapsed
		 * (similar as #currentVisibleItems). (this is only necessary in the
		 * case (isTree || hasDifferingHeights))
		 */
	}

	/**
	 * Returns the height of the on-screen area that is available for showing
	 * the grid's rows, i.e. the client area of the scrollable minus the height
	 * of the column headers (if shown).
	 * 
	...
	/**
	 * Returns the height of the screen area that is available for showing the
	 * grid columns
	 * 
	 * @return
	 */
	 ...
	 * enabled, returns true if the item at the given index contains at least
	 * one selected cell.
	 * 
	 * @param index
	...
	public boolean isSelected(int index) {
		...
		if (!cellSelectionEnabled) {
			return isSelected(getItem(index));
		} else {
			for (Iterator iter = selectedCells.iterator(); iter.hasNext();) {
				Point cell = (Point) iter.next();
				if (cell.y == index) {
	 ...
	 * <p>
	 * If cell selection is enabled, all cells within the given range are
	 * selected.
	 * 
	 * @param start
	 *            the start of the range
	 ...
	 * <p>
	 * If the receiver is single-select, do nothing. If cell selection is
	 * enabled, all cells are selected.
	 * 
	 * @throws org.eclipse.swt.SWTException
	 *             <ul>
	 *             <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed
		...
		if (show && isAutoWidth()) {
			...
			for (Iterator iter = itemIterator(); iter.hasNext();) {
				...
				rowHeaderWidth = Math.max(rowHeaderWidth, rowHeaderRenderer
						.computeSize(sizingGC, SWT.DEFAULT, SWT.DEFAULT,
								iterItem).x);
			}
		}
	 ...
	 * <p>
	 * If cell selection is enabled, all cells within the item at the given
	 * index are selected.
	 * 
	 * @param index
	 *            the index of the item to select
	...
	public void setSelection(int index) {
		...
		if (index >= 0 && index < getItemCount()) {
			if (!cellSelectionEnabled) {
				...
				selectedCells.clear();
				selectCells(getCells(getItem(index)));
			}
		}
	}
	...
	public void showColumn(GridColumn col) {
		...
		if (!getColumnScrolling()) {
			if (x < firstVisibleX) {
				hScroll.setSelection(getHScrollSelectionInPixels()
						- (firstVisibleX - x));
			} else {
			...
			if (x < firstVisibleX
					|| col.getWidth() > getClientArea().width - firstVisibleX) {
				int sel = displayOrderedColumns.indexOf(col);
				hScroll.setSelection(sel);
			} else {
				int availableWidth = getClientArea().width - firstVisibleX
						- col.getWidth();
				...
				while (true) {
					if (prevCol == null || prevCol.getWidth() > availableWidth) {
						int sel = displayOrderedColumns
								.indexOf(currentScrollTo);
						hScroll.setSelection(sel);
						break;
					} else {
	...
	/**
	 * Returns true if 'item' is currently being <em>completely</em> shown in
	 * this <code>Grid</code>'s visible on-screen area.
	 * 
	 * <p>
	 * Here, "completely" only refers to the item's height, not its width. This
	 * means this method returns true also if some cells are horizontally
	 * scrolled away.
	 * 
	 * @param item
	 ...
	 *             <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed
	 *             </li> <li>ERROR_THREAD_INVALID_ACCESS - if not called from
	 *             the thread that created the receiver</li> <li>
	 *             ERROR_INVALID_ARGUMENT - if 'item' is not contained in the
	 *             receiver</li>
	 *             </ul>
	 */
	boolean isShown(GridItem item) {
		...
		int itemIndex = getIndexOfItem(item);
	 ...
	 *             <li>ERROR_THREAD_INVALID_ACCESS - if not called from the
	 *             thread that created the receiver</li>
	 *             <li>ERROR_INVALID_ARGUMENT - if 'item' is not contained in
	 *             the receiver</li>
	 *             </ul>
	 */
	...
	private void computeHeaderHeight(GC gc) {
		...
		for (Iterator<GridColumn> columnsIterator = columns.iterator(); columnsIterator
				.hasNext();) {
			GridColumn column = columnsIterator.next();
			colHeaderHeight = Math.max(
					column.getHeaderRenderer().computeSize(gc,
							column.getWidth(), SWT.DEFAULT, column).y,
					colHeaderHeight);
		}
		...
		for (int groupIndex = 0; groupIndex < columnGroups.length; groupIndex++) {
			...
			groupHeight = Math.max(
					group.getHeaderRenderer().computeSize(gc, SWT.DEFAULT,
							SWT.DEFAULT, group).y, groupHeight);
		}
	...
	private void computeFooterHeight(GC gc) {
		...
		for (Iterator columnsIterator = columns.iterator(); columnsIterator
				.hasNext();) {
			GridColumn column = (GridColumn) columnsIterator.next();
			colFooterHeight = Math.max(
					column.getFooterRenderer().computeSize(gc,
							column.getWidth(), SWT.DEFAULT, column).y,
					colFooterHeight);
		}
	...
	/**
	 * Returns the computed default item height. Currently this method just gets
	 * the preferred size of all the cells in the given row and returns that (it
	 * is then used as the height of all rows with items having a height of -1).
	 * 
	 * @param item
	...
	private int computeItemHeight(GridItem item, GC gc) {
		...
		if (columns.size() == 0 || getItemCount() == 0) {
			return height;
		}
		...
		for (Iterator columnsIterator = columns.iterator(); columnsIterator
			...
			height = Math.max(
					height,
					column.getCellRenderer().computeSize(gc, SWT.DEFAULT,
							SWT.DEFAULT, item).y);
		}

		if (rowHeaderVisible && rowHeaderRenderer != null) {
			height = Math.max(height, rowHeaderRenderer.computeSize(gc,
	...
	private int getColumnHeaderXPosition(GridColumn column) {
		...
		for (Iterator column2Iterator = displayOrderedColumns.iterator(); column2Iterator
				.hasNext();) {
	...
	private Point getTableSize() {
		...
		for (Iterator columnIterator = columns.iterator(); columnIterator
				.hasNext();) {
	...
	private boolean handleColumnDragging(int x) {
		...
		if (x < x2) {
			for (Iterator columnIterator = displayOrderedColumns.iterator(); columnIterator
					.hasNext();) {
				GridColumn column = (GridColumn) columnIterator.next();
				if (!column.isVisible()) {
					continue;
			...
			if (columnGroups.length != 0) {
				...
				if (columnBeingPushed.getColumnGroup() == null) {
					...
					if (!(local_dragDropBeforeColumn != null && local_dragDropBeforeColumn
							.getColumnGroup() == columnBeingPushed
							.getColumnGroup())
		...
		redraw(clientArea.x, clientArea.y, clientArea.width, clientArea.height,
				false);
		...
		if ((dragDropBeforeColumn != columnBeingPushed && dragDropAfterColumn != columnBeingPushed)
			...
			if (dragDropBeforeColumn == null) {
				...
				if (columnGroups.length != 0) {
					...
					if (dragDropBeforeColumn.getColumnGroup() == columnBeingPushed
							.getColumnGroup()) {
						insertAtIndex = displayOrderedColumns
								.indexOf(dragDropBeforeColumn);
					} else if (dragDropAfterColumn.getColumnGroup() == columnBeingPushed
							.getColumnGroup()) {
						insertAtIndex = displayOrderedColumns
								.indexOf(dragDropAfterColumn) + 1;
					} else {
						if (dragDropBeforeColumn.getColumnGroup() == null) {
							insertAtIndex = displayOrderedColumns
									.indexOf(dragDropBeforeColumn);
						} else {
							GridColumnGroup beforeGroup = dragDropBeforeColumn
									.getColumnGroup();
							insertAtIndex = displayOrderedColumns
									.indexOf(dragDropBeforeColumn);
							while (insertAtIndex > 0
									&& displayOrderedColumns.get(
											insertAtIndex - 1).getColumnGroup() == beforeGroup) {
								insertAtIndex--;
							}
	...
	private boolean handleColumnGroupHeaderClick(int x, int y) {
		...
		overThis.getHeaderRenderer().setBounds(
				headerX - getHScrollSelectionInPixels(), 0, width,
				groupHeaderHeight);
		return overThis.getHeaderRenderer().notify(
				IInternalWidget.LeftMouseButtonDown, new Point(x, y), overThis);
	}
	 ...
	 * @return true if this event has been consumed.
	 */
	private boolean handleHoverOnColumnHeader(int x, int y) {
...
//		boolean overSorter = false;
		...
		if (y <= headerHeight) {
			...
			for (Iterator<GridColumn> columnIterator = displayOrderedColumns.iterator(); columnIterator
				...
				if (x2 >= (x - COLUMN_RESIZER_THRESHOLD)
						&& x2 <= (x + COLUMN_RESIZER_THRESHOLD)) {
					if (column.getResizeable()) {
						if (column.getColumnGroup() != null
								&& y <= groupHeaderHeight) {
							// if this is not the last column
							if (column != column.getColumnGroup()
...
//				else {
//					if(isOverSortArrow(column,x-x2)){
//						overSorter = true;
...
//			if(overSorter){
//				setCursor(getColumnHoverCursor(columnBeingSorted));
//			}else{
...
//		}
	...
	Cursor getColumnHoverCursor(GridColumn columnBeingResized2) {
		return getDisplay().getSystemCursor(SWT.CURSOR_SIZEWE);
	}
		...
		if (x <= rowHeaderWidth) {
			...
			while (row < getItemCount() && y2 <= getClientArea().height) {
				...
				if (currItem.isVisible()) {
					...
					if (y2 >= (y - ROW_RESIZER_THRESHOLD)
							&& y2 <= (y + ROW_RESIZER_THRESHOLD)) {
						...
						if (rowBeingResized != null) {
							// we have passed all (overlapping) row resizers, so
							// break
							break;
						}
					}
	...
	public Point getCell(Point point) {
		...
		if (item != null && column != null) {
			return new Point(columns.indexOf(column), getIndexOfItem(item));
		} else {
			return null;
		}
	}
	...
	protected void onPaint(PaintEvent e) {
		int insertMarkPosX1 = -1; // we will populate these values while drawing
									// the cells
		...
		drawBackground(e.gc, 0, 0, getSize().x, getSize().y);
		...
		if (getItemCount() > 0 && availableHeight > 0) {
			RowRange range = getRowRange(getTopIndex(), availableHeight, false,
					false);
			if (range.height >= availableHeight) {
				visibleRows = range.rows;
			} else {
				visibleRows = range.rows + (availableHeight - range.height)
						/ getItemHeight() + 1;
			}
		}
		...
		if (hasSpanning) {
			// We need to find the first Item to draw. An earlier item can
			// row-span the first visible item.
			for (int rowIndex = 0; rowIndex < firstVisibleIndex; rowIndex++) {
				GridItem itemForRow = getItem(rowIndex);
				int colIndex = 0;
				...
				for (Iterator columnIterator = displayOrderedColumns.iterator(); columnIterator
					...
					maxRowSpanForItem = rowSpan > maxRowSpanForItem ? rowSpan
							: maxRowSpanForItem;
				}
		...
		for (int i = 0; i < visibleRows + (firstVisibleIndex - firstItemToDraw); i++) {
			...
			if ((row >= 0) && (row < getItemCount())) {
				...
				while (!item.isVisible() && row < getItemCount() - 1) {
					row++;
					item = getItem(row);
				}
			}
			...
			if (item != null) {
				...
				for (Iterator<GridColumn> columnIterator = displayOrderedColumns.iterator(); columnIterator
					...
					if (skipCell == false) {
						...
						if (nrRowsToSpan > 0 || nrColumnsToSpan > 0) {
							cellSpanManager.addCellSpanInfo(colIndex, row,
									nrColumnsToSpan, nrRowsToSpan);
						}

						if (x + width >= 0 && x < getClientArea().width) {
							Point sizeOfColumn = item
									.getCellSize(indexOfColumn);

							column.getCellRenderer().setBounds(x, y, width,
									sizeOfColumn.y);
							int cellInHeaderDelta = columnHeadersVisible ? headerHeight
									- y
									: 0;
							if (cellInHeaderDelta > 0) {
								e.gc.setClipping(new Rectangle(x - 1, y
										+ cellInHeaderDelta, width + 1,
										sizeOfColumn.y + 2 - cellInHeaderDelta));
							} else {
								e.gc.setClipping(new Rectangle(x - 1, y - 1,
										width + 1, sizeOfColumn.y + 2));
							}
							...
							column.getCellRenderer().setCellFocus(cellSelectionEnabled && focusItem == item
											&& focusColumn == column);
							...
							if (selectedCells.contains(new Point(indexOfColumn,
									row))) {
							...
							if (!insertMarkPosFound
								...
								if (insertMarkColumn == null) {
									insertMarkPosX2 = getClientArea().x
											+ getClientArea().width;
								} else {
				...
				if (x < getClientArea().width) {
					...
					emptyCellRenderer.setBounds(x, y, getClientArea().width - x
							+ 1, item.getHeight());
				...
				if (rowHeaderVisible) {
					...
					if (!cellSelectionEnabled) {
						rowHeaderRenderer.setSelected(selectedItems
								.contains(item));
					} else {
				...
				if (isFocusControl() && !cellSelectionEnabled) {
					if (item == focusItem) {
						if (focusRenderer != null) {
							...
							focusRenderer.setBounds(focusX, focusY - 1,
									getClientArea().width - focusX - 1,
									item.getHeight() + 1);
				...
				for (Iterator<GridColumn> columnIterator = displayOrderedColumns.iterator(); columnIterator
					...
					if (column.isVisible()) {
						emptyCellRenderer.setBounds(x, y, column.getWidth(),
								getItemHeight());
				...
				if (rowHeaderVisible) {
					emptyRowHeaderRenderer.setBounds(x, y, rowHeaderWidth,
							getItemHeight() + 1);
		...
		if (draggingColumn) {
			if ((dragDropAfterColumn != null || dragDropBeforeColumn != null)
				...
				Point size = dropPointRenderer.computeSize(e.gc, SWT.DEFAULT,
						SWT.DEFAULT, null);
				x -= size.x / 2;
				if (x < 0) {
					x = 0;
				}
				dropPointRenderer.setBounds(x - 1, headerHeight
						+ DROP_POINT_LOWER_OFFSET, size.x, size.y);
		...
		// draw insertion mark
		if (insertMarkPosFound) {
			e.gc.setClipping(rowHeaderVisible ? rowHeaderWidth : 0,
					columnHeadersVisible ? headerHeight : 0,
					getClientArea().width, getClientArea().height);
			insertMarkRenderer.paint(e.gc, new Rectangle(insertMarkPosX1,
					insertMarkPosY, insertMarkPosX2 - insertMarkPosX1, 0));
		}
	...
	private void paintHeader(GC gc) {
		...
		for (Iterator<GridColumn> columnIterator = displayOrderedColumns.iterator(); columnIterator
			...
			if (column.getColumnGroup() != null) {
				...
				if (column.getColumnGroup() != previousPaintedGroup) {
					...
					if (displayOrderedColumns.indexOf(column) + 1 < displayOrderedColumns
							.size()) {
					...
					while (nextCol != null
							&& nextCol.getColumnGroup() == column
									.getColumnGroup()) {

						if ((nextCol.getColumnGroup().getExpanded() && !nextCol
								.isDetail())
								|| (!nextCol.getColumnGroup().getExpanded() && !nextCol
										.isSummary())) {
					...
					for (int i = 0; i < column.getColumnGroup().getColumns().length; i++) {
						...
						if (col.isVisible()
								&& (column.getMoveable() || !selectedColumns
										.contains(col))) {
							selected = false;
							break;
						}
					...
					column.getColumnGroup()
							.getHeaderRenderer()
							.setHover(
									hoverColumnGroupHeader == column
											.getColumnGroup());
					column.getColumnGroup().getHeaderRenderer()
							.setHoverDetail(hoveringDetail);

					column.getColumnGroup().getHeaderRenderer()
							.setBounds(x, 0, width, groupHeaderHeight);

					column.getColumnGroup().getHeaderRenderer()
							.paint(gc, column.getColumnGroup());
			...
			if (pushingColumn) {
				column.getHeaderRenderer().setHover(
						columnBeingPushed == column && pushingAndHovering);
			} else {
				column.getHeaderRenderer().setHover(
						hoveringColumnHeader == column);
			}

			column.getHeaderRenderer().setHoverDetail(hoveringDetail);

			column.getHeaderRenderer().setBounds(x, y, column.getWidth(),
					height);

			if (cellSelectionEnabled) {
				column.getHeaderRenderer().setSelected(
		...
		if (x < getClientArea().width) {
			emptyColumnHeaderRenderer.setBounds(x, 0,
					getClientArea().width - x, headerHeight);
	...
	private void paintFooter(GC gc) {
		...
		for (Iterator columnIterator = displayOrderedColumns.iterator(); columnIterator
			...
			column.getFooterRenderer().setBounds(x, y, column.getWidth(),
					height);
			if (x + column.getWidth() >= 0) {
		...
		if (x < getClientArea().width) {
			emptyColumnFooterRenderer.setBounds(x, getClientArea().height
					- footerHeight, getClientArea().width - x, footerHeight);
			emptyColumnFooterRenderer.paint(gc, null);
		}

		if (rowHeaderVisible) {
			// paint left corner
			bottomLeftRenderer.setBounds(0, getClientArea().height
					- footerHeight, rowHeaderWidth, footerHeight);
	...
	void updateScrollbars() {
		...
		if (vScroll.getVisible()) {
			...
			if (!hasDifferingHeights) {
				// in this case, the number of visible rows on screen is
				// constant,
				...
				// in this case, the number of visible rows on screen is
				// variable,
				// so we have to use 1 as thumb and decrease max by the number
				// of
				// rows on the last page
				if (getVisibleGridHeight() >= 1) {
					RowRange range = getRowRange(-1, getVisibleGridHeight(),
							true, true);
					max -= range.rows - 1;
				}
			}
		...
		if (hScroll.getVisible()) {
			...
			if (!columnScrolling) {
				...
				// make it the max you can
				int selection = Math
						.min(hScroll.getSelection(), hiddenArea - 1);

				hScroll.setValues(selection, 0, hiddenArea + clientArea.width
						- 1, clientArea.width, HORZ_SCROLL_INCREMENT,
						clientArea.width);
			} else {
	...
	private Event updateSelection(GridItem item, int stateMask) {
		...
		if (selectionType == SWT.SINGLE) {
			...
			Rectangle clientArea = getClientArea();
			redraw(clientArea.x, clientArea.y, clientArea.width,
			...
			if (!shift && !ctrl) {
				...
				int anchorIndex = getIndexOfItem(shiftSelectionAnchorItem);
				int itemIndex = getIndexOfItem(item);
				...
				for (int i = min; i <= max; i++) {
					if (!selectedItems.contains(getItem(i))
							&& getItem(i).isVisible()) {
	 ...
	 */
	private Event updateCellSelection(Point newCell, int stateMask,
			boolean dragging, boolean reverseDuplicateSelections) {
		Vector v = new Vector();
		v.add(newCell);
		return updateCellSelection(v, stateMask, dragging,
				reverseDuplicateSelections);
	}
	 ...
	 * @return selection event that will need to be fired or null.
	 */
	private Event updateCellSelection(Vector newCells, int stateMask,
			boolean dragging, boolean reverseDuplicateSelections) {
		boolean shift = false;
		boolean ctrl = false;
		...
		if (!shift && !ctrl) {
			...
			Point newCell = (Point) newCells.get(0); // shift selection should
														// only occur with one
			...
			Point newRange = getSelectionRange(currentItem, currentColumn,
					endItem, endColumn);
			...
			do {
				...
				do {
					if (!firstLoop2) {
						int index = displayOrderedColumns
								.indexOf(currentColumn) + 1;

						if (index < displayOrderedColumns.size()) {
							currentColumn = getVisibleColumn_DegradeRight(
									currentItem,
					...
					if (currentColumn != null) {
						Point cell = new Point(indexOf(currentColumn),
								indexOf(currentItem));
	...
	private void onFocusIn() {
		if (getItemCount()!=0 && focusItem == null) {// MMMM
			focusItem = getItem(0);
		}
	}

	private void onDispose(Event event) {
		// We only want to dispose of our items and such *after* anybody else
		// who may have been
		...
		for (Iterator<GridItem> iterator = itemIterator(); iterator.hasNext();) {
		...
		for (Iterator<GridColumn> iterator = columns.iterator(); iterator
				.hasNext();) {
			GridColumn col = iterator.next();
			col.dispose();
		}
...
//		}
		...
		if (e.button == 1 && handleColumnHeaderPush(e)) {
			return;
		}
		...
		if (isListening(SWT.DragDetect)) {
			if ((cellSelectionEnabled && hoveringOnSelectionDragArea)
					|| (!cellSelectionEnabled && item != null && selectedItems
							.contains(item))) {
				if (dragDetect(e)) {
					return;
				}
		...
		if (item != null) {
			if (cellSelectionEnabled) {
				...
				if (col != null) {
					isSelectedCell = selectedCells.contains(new Point(
							indexOf(col), indexOf(item)));
				}

				if (e.button == 1
						|| (e.button == 3 && col != null && !isSelectedCell)) {
					if (col != null) {
						selectionEvent = updateCellSelection(new Point(
								indexOf(col), indexOf(item)), e.stateMask,
								false, true);
						...
						if (e.x <= rowHeaderWidth) {
							...
							selectionEvent = updateCellSelection(cells,
									newStateMask, shift, ctrl);
							cellRowSelectedOnLastMouseDown = (getCellSelectionCount() > 0);

							if (!shift) {
								// set focus back to the first visible column
								focusColumn = getColumn(new Point(
										rowHeaderWidth + 1, e.y));
		...
		} else if (e.button == 1 && rowHeaderVisible && e.x <= rowHeaderWidth
				&& e.y < headerHeight) {
			// Nothing to select
			if (getItemCount() == 0) {
				return;
		...
		} else if (cellSelectionEnabled && e.button == 1
				&& columnHeadersVisible && e.y <= headerHeight) {
			...
			selectionEvent = updateCellSelection(cells, e.stateMask, false,
					true);
			cellColumnSelectedOnLastMouseDown = (getCellSelectionCount() > 0);

			GridItem newFocusItem = getItem(0);

			while (newFocusItem != null
					&& getSpanningColumn(newFocusItem, col) != null) {
	...
	private void onMouseDoubleClick(MouseEvent e) {
		if (e.button == 1) {
			if (hoveringOnColumnResizer) {
				...
				columnBeingResized.fireResized();
				for (int index = displayOrderedColumns
						.indexOf(columnBeingResized) + 1; index < displayOrderedColumns
						.size(); index++) {
					GridColumn col = displayOrderedColumns.get(index);
					if (col.isVisible()) {
						col.fireMoved();
...
//		if (hoveringOnColumnSorter) {
//			handleHoverOnColumnHeader(e.x, e.y);
////			if (hoveringOnColumnSorter && (e.button == 1)) {
...
//		}
		...
		if (cellDragSelectionOccuring || cellRowDragSelectionOccuring
	...
	private void onMouseMove(MouseEvent e) {
		...
		if (inplaceTooltipCapture
				&& (e.x < 0 || e.y < 0 || e.x >= getBounds().width || e.y >= getBounds().height)) {
			setCapture(false);
		...
		if ((e.stateMask & SWT.BUTTON1) == 0) {
			...
			if (cellSelectionEnabled) {
				...
				if (!cellRowDragSelectionOccuring
				...
				if (!cellColumnDragSelectionOccuring
				...
				if (cellDragSelectionOccuring && handleCellHover(e.x, e.y)) {
					...
					if (hoveringColumn == null) {
						if (e.x > rowHeaderWidth) {
							...
							intentColumn = getVisibleColumn_DegradeLeft(
									intentItem,
									displayOrderedColumns
											.get(displayOrderedColumns.size() - 1));
						} else {
					...
					showItem(intentItem);
					selectionEvent = updateCellSelection(new Point(
							indexOf(intentColumn), indexOf(intentItem)),
				...
				if (cellRowDragSelectionOccuring && handleCellHover(e.x, e.y)) {
					...
					if (hoveringItem == null) {
						if (e.y > headerHeight) {
							...
							if (getTopIndex() > 0) {
								intentItem = getPreviousVisibleItem(getItem(getTopIndex()));
							} else {
								intentItem = getItem(0);
							}
						}
					...
					showItem(intentItem);
					selectionEvent = updateCellSelection(cells, ctrlFlag, true,
							false);
				}
				if (cellColumnDragSelectionOccuring
						&& handleCellHover(e.x, e.y)) {
					...
					boolean decreasing = (displayOrderedColumns
							.indexOf(iterCol) > displayOrderedColumns
							.indexOf(focusColumn));
					...
					selectionEvent = updateCellSelection(newSelected, ctrlFlag,
							true, false);
				}
	...
	private void handleHovering(int x, int y) {
		...
		if (columnHeadersVisible) {
			if (handleHoverOnColumnHeader(x, y)) {
				// if (hoveringItem != null || !hoveringDetail.equals("") ||
				// hoveringColumn != null
				// || hoveringColumnHeader != null || hoverColumnGroupHeader !=
				// null)
				// {
				// hoveringItem = null;
				// hoveringDetail = "";
	...
	protected void refreshHoverState() {
		Point p = getDisplay()
				.map(null, this, getDisplay().getCursorLocation());
	...
	private void onKeyDown(Event e) {
		...
		if ((e.character == '-' || (!cellSelectionEnabled && e.keyCode == SWT.ARROW_LEFT))
				&& focusItem != null && focusItem.isExpanded()) {
			attemptExpandCollapse = SWT.Collapse;
		} else if ((e.character == '+' || (!cellSelectionEnabled && e.keyCode == SWT.ARROW_RIGHT))
				&& focusItem != null && !focusItem.isExpanded()) {
			attemptExpandCollapse = SWT.Expand;
		}

		if (attemptExpandCollapse != 0 && focusItem != null
				&& focusItem.hasChildren()) {
			int performExpandCollapse = 0;

			if (cellSelectionEnabled && focusColumn != null
					&& focusColumn.isTree()) {
		...
		// These two variables are used because the key navigation when the
		// shift key is down is
		// based, not off the focus item/column, but rather off the implied
		// focus (i.e. where the
		...
		case SWT.ARROW_RIGHT:
			if (cellSelectionEnabled) {
				if (impliedFocusItem != null && impliedFocusColumn != null) {
					...
					int index = displayOrderedColumns
							.indexOf(impliedFocusColumn);

					int jumpAhead = impliedFocusItem
							.getColumnSpan(indexOf(impliedFocusColumn));
			...
			if (cellSelectionEnabled) {
				if (impliedFocusItem != null && impliedFocusColumn != null) {
					...
					if (index != 0) {
						...
						newColumnFocus = getVisibleColumn_DegradeLeft(
								impliedFocusItem, newColumnFocus);
					} else {
						newColumnFocus = impliedFocusColumn;
					}
				}
				...
				if (impliedFocusItem != null
						&& impliedFocusItem.getParentItem() != null) {
		...
		case SWT.HOME:
			...
			if (!cellSelectionEnabled) {
				...
				newSelection = impliedFocusItem;
				newColumnFocus = getVisibleColumn_DegradeRight(newSelection,
						displayOrderedColumns.get(0));
			}
		...
		case SWT.PAGE_UP:
			...
			newSelection = getItem(topIndex);

			if (focusItem == newSelection) {
				RowRange range = getRowRange(getTopIndex(),
						getVisibleGridHeight(), false, true);
				newSelection = getItem(range.startIndex);
			}
		...
		case SWT.PAGE_DOWN:
			...
			newSelection = getItem(bottomIndex);
			...
			if (focusItem == newSelection) {
				RowRange range = getRowRange(getBottomIndex(),
						getVisibleGridHeight(), true, false);
				newSelection = getItem(range.endIndex);
			}
		...
		if (cellSelectionEnabled) {
			...
			if (e.stateMask != SWT.MOD1) {
				Event selEvent = updateCellSelection(new Point(
						indexOf(newColumnFocus), indexOf(newSelection)),
	...
	private void handleSpaceBarDown(Event event) {
		...
		if (selectionEnabled && !cellSelectionEnabled
				&& !selectedItems.contains(focusItem)) {
	...
	private void onScrollSelection() {
		...
		bottomIndex = -1;
		refreshHoverState();
		redraw(getClientArea().x, getClientArea().y, getClientArea().width,
	...
	Point getOrigin(GridColumn column, GridItem item) {
		...
		for (Iterator colIterIterator = displayOrderedColumns.iterator(); colIterIterator
				.hasNext();) {
		...
		if (item != null) {
			...
			while (currIndex != itemIndex) {
				if (currIndex < itemIndex) {
					GridItem currItem = getItem(currIndex);
					if (currItem.isVisible()) {
	...
	private boolean handleCellClick(GridItem item, int x, int y) {
		...
		return col.getCellRenderer().notify(
				IInternalWidget.LeftMouseButtonDown, new Point(x, y), item);
	...
	private boolean handleCellHover(int x, int y) {
		...
		if (col != null) {
			if (item != null) {
				if (y < getClientArea().height - footerHeight) {
					col.getCellRenderer().setBounds(
							item.getBounds(columns.indexOf(col)));

					if (col.getCellRenderer().notify(IInternalWidget.MouseMove,
							new Point(x, y), item)) {
						detail = col.getCellRenderer().getHoverDetail();
					}

					Rectangle textBounds = col.getCellRenderer().getTextBounds(
							item, false);

					if (textBounds != null) {
						Point p = new Point(x
								- col.getCellRenderer().getBounds().x, y
				...
				if (y < headerHeight) {
					if (columnGroups.length != 0 && y < groupHeaderHeight
						...
						hoverColGroup.getHeaderRenderer().setBounds(
								hoverColGroup.getBounds());
						if (hoverColGroup.getHeaderRenderer().notify(
								IInternalWidget.MouseMove, new Point(x, y),
								hoverColGroup)) {
							detail = hoverColGroup.getHeaderRenderer()
									.getHoverDetail();
						}

						Rectangle textBounds = hoverColGroup
								.getHeaderRenderer().getTextBounds(
										hoverColGroup, false);

						if (textBounds != null) {
							Point p = new Point(x
									- hoverColGroup.getHeaderRenderer()
											.getBounds().x, y
									- hoverColGroup.getHeaderRenderer()
											.getBounds().y);
							overText = textBounds.contains(p);
						...
						if (col.getHeaderRenderer()
								.notify(IInternalWidget.MouseMove,
										new Point(x, y), col)) {
							detail = col.getHeaderRenderer().getHoverDetail();
						}

						Rectangle textBounds = col.getHeaderRenderer()
								.getTextBounds(col, false);

						if (textBounds != null) {
							Point p = new Point(x
									- col.getHeaderRenderer().getBounds().x, y
		...
		if (hoveringItem != item || !hoveringDetail.equals(detail)
				|| hoveringColumn != col
				|| hoverColGroup != hoverColumnGroupHeader
		...
		if (hoverChange || hoveringOverText != overText) {
			...
			if (overText) {
				...
				if (hoveringItem != null
						&& hoveringItem.getToolTipText(indexOf(col)) == null && // no
																				// inplace
																				// tooltips
																				// when
																				// regular
																				// tooltip
						!col.getWordWrap()) // dont show inplace tooltips for
											// cells with wordwrap
				{
					...
					textBounds = col.getCellRenderer().getTextBounds(item,
							false);
					preferredTextBounds = col.getCellRenderer().getTextBounds(
							item, true);
				} else if (hoveringColumnHeader != null
						&& hoveringColumnHeader.getHeaderTooltip() == null) // no
																			// inplace
																			// tooltips
																			// when
																			// regular
																			// tooltip
				{
					cellBounds = hoveringColumnHeader.getHeaderRenderer()
							.getBounds();
					if (cellBounds.x + cellBounds.width > getSize().x) {
						cellBounds.width = getSize().x - cellBounds.x;
					}
					textBounds = hoveringColumnHeader.getHeaderRenderer()
							.getTextBounds(col, false);
					preferredTextBounds = hoveringColumnHeader
							.getHeaderRenderer().getTextBounds(col, true);
				} else if (hoverColumnGroupHeader != null) {
					cellBounds = hoverColumnGroupHeader.getHeaderRenderer()
							.getBounds();
					if (cellBounds.x + cellBounds.width > getSize().x) {
						cellBounds.width = getSize().x - cellBounds.x;
					}
					...
					preferredTextBounds = hoverColumnGroupHeader
							.getHeaderRenderer().getTextBounds(
				...
				// if we are truncated
				if (textBounds != null
						&& textBounds.width < preferredTextBounds.width) {
					showToolTip(item, col, hoverColumnGroupHeader, new Point(
							cellBounds.x + textBounds.x, cellBounds.y
									+ textBounds.y));
					// the following 2 lines are done here rather than in
					// showToolTip to allow
		...
		if (hoverChange) {
			...
			if (newTip == null) { // no cell or column header specific tooltip
									// then use base Grid tooltip
				newTip = getToolTipText();
			}

			// Avoid unnecessarily resetting tooltip - this will cause the
			// tooltip to jump around
	...
	/**
	 * Sets the tooltip for the whole Grid to the given text. This method is
	 * made available for subclasses to override, when a subclass wants to
	 * display a different than the standard SWT/OS tooltip. Generally, those
	 * subclasses would override this event and use this tooltip text in their
	 * own tooltip or just override this method to prevent the SWT/OS tooltip
	 * from displaying.
	 * 
	 * @param text
	 */
	...
	int newColumn(GridColumn column, int index) {
		...
		for (Iterator iterator = itemIterator(); iterator.hasNext();) {
	...
	void removeColumn(GridColumn column) {
		...
		if (cellSelectionEnabled) {
			...
			for (Iterator iterator = selectedCells.iterator(); iterator
					.hasNext();) {
	...
	/**
	 * Manages the setting of the checkbox column when the SWT.CHECK style was
	 * given to the table. This method will ensure that the first column of the
	 * table always has a checkbox when SWT.CHECK is given to the table.
	 */
	...
	int newItem(GridItem item, int index, boolean root) {
		...
		// Have to convert indexes, this method needs a flat index, the method
		// is called with indexes
		// that are relative to the level
		if (root && index != -1) {
			if (index >= rootItems.size()) {
				index = -1;
			} else {
				index = getIndexOfItem(rootItems.get(index));
			}
		} else if (!root) {
		...
		if (getItemCount() == 1 && !userModifiedItemHeight) {
	...
	void newColumnGroup(GridColumnGroup group) {
		...
		System.arraycopy(columnGroups, 0, newColumnGroups, 0,
				columnGroups.length);
	...
	/**
	 * Returns the current cell in focus. If cell selection is disabled, this
	 * method returns null.
	 * 
	 * @return cell in focus or {@code null}. x represents the column and y the
	 *         row the cell is in
	 * @throws org.eclipse.swt.SWTException
	 *             <ul>
	 *             <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed
	...
	/**
	 * Sets the focused item to the given column. Column focus is only
	 * applicable when cell selection is enabled.
	 * 
	 * @param column
	 *            column to focus.
	 * @throws IllegalArgumentException
	 ...
	 *            true to horizontally scroll by column, false to scroll by
	 *            pixel
	 * @throws org.eclipse.swt.SWTException
	 *             <ul>
	 *             <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed
	...
	/**
	 * Returns the first visible column that is not spanned by any other column
	 * that is either the given column or any of the columns displaying to the
	 * left of the given column. If the given column and subsequent columns to
	 * the right are either not visible or spanned, this method will return
	 * null.
	 * 
	 * @param item
	 * @param col
	 * @return
	 */
	private GridColumn getVisibleColumn_DegradeLeft(GridItem item,
			GridColumn col) {
	 ...
	 * Returns the first visible column that is not spanned by any other column
	 * that is either the given column or any of the columns displaying to the
	 * right of the given column. If the given column and subsequent columns to
	 * the right are either not visible or spanned, this method will return
	 * null.
	 * 
	 * @param item
	 ...
	 * @return
	 */
	private GridColumn getVisibleColumn_DegradeRight(GridItem item,
			GridColumn col) {
		int index = displayOrderedColumns.indexOf(col);
		...
		while (index > 0) {
			...
			if (item.getColumnSpan(indexOf(prevCol)) >= startIndex - index) {
				if (startIndex == displayOrderedColumns.size() - 1) {
					...
					return getVisibleColumn_DegradeRight(item,
							displayOrderedColumns.get(startIndex + 1));
				}
			}
	 ...
	 * 
	 * @param cells
	 *            an arry of points whose x value is a column index and y value
	 *            is an item index
	 * @throws IllegalArgumentException
	 *             <ul>
	 *             <li>ERROR_NULL_ARGUMENT - if the set of cells or an
	 *             individual cell is null</li>
	 *             </ul>
	 * @throws org.eclipse.swt.SWTException
	 *             <ul>
	 *             <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed
	...
	private Event selectAllCellsInternal() {
		...
		Event event = updateCellSelection(new Point(indexOf(lastCol),
				indexOf(lastItem)), SWT.MOD2, true, false);
	...
	/**
	 * Selects the selection to the given cell. The existing selection is
	 * cleared before selecting the given cell.
	 * 
	 * @param cell
	 *            point whose x values is a column index and y value is an item
	 *            index
	 ...
	 * @throws IllegalArgumentException
	 *             <ul>
	 *             <li>ERROR_NULL_ARGUMENT - if the cell array or an individual
	 *             cell is null</li>
	 ...
	 * The order of the items is unspecified. An empty array indicates that no
	 * items are selected.
	 * <p>
	 * Note: This is not the actual structure used by the receiver to maintain
	 * its selection, so modifying the array will not affect the receiver.
	...
	void updateColumnFocus() {
		if (!focusColumn.isVisible()) {
			...
			if (index > 0) {
				...
				focusColumn = getVisibleColumn_DegradeRight(focusItem,
						focusColumn);
			}
		}
	}
	...
	private void getCells(GridColumn col, Vector cells) {
		...
		while (item != null) {
			...
			for (Iterator iter = displayOrderedColumns.iterator(); iter
					.hasNext();) {
				GridColumn nextCol = (GridColumn) iter.next();
				if (!nextCol.isVisible()) {
					continue;
	...
	/**
	 * Returns a point whose x and y values are the to and from column indexes
	 * of the new selection range inclusive of all spanned columns.
	 * 
	 * @param fromItem
	 * @param fromColumn
	 * @param toItem
	 ...
	 * @return
	 */
	private Point getSelectionRange(GridItem fromItem, GridColumn fromColumn,
			GridItem toItem, GridColumn toColumn) {
		if (displayOrderedColumns.indexOf(fromColumn) > displayOrderedColumns
				.indexOf(toColumn)) {
		...
		do {
			...
			if (cols.x != fromIndex || cols.y != toIndex) {
				...
				// Unfortunately we have to start all over again from the top
				// with the new range
	...
	/**
	 * Returns a point whose x and y value are the to and from column indexes of
	 ...
	 * @return
	 */
	private Point getRowSelectionRange(GridItem item, GridColumn fromColumn,
			GridColumn toColumn) {
	...
	/**
	 * Returns the column which is spanning the given column for the given item
	 * or null if it is not being spanned.
	 * 
	 * @param item
	 * @param column
	 * @return
	...
	/**
	 * Shows the inplace tooltip for the given item and column. The location is
	 * the x and y origin of the text in the cell.
	 * <p>
	...
	protected void showToolTip(GridItem item, GridColumn column,
		...
		if (group != null) {
			...
			inplaceToolTip.setFont(item.getFont(item.getParent()
					.indexOf(column)));
			inplaceToolTip.setText(item.getText(item.getParent()
					.indexOf(column)));
		} else if (column != null) {
			inplaceToolTip.setFont(getFont());
	 ...
	 * <p>
	 * This method must be overriden when showToolTip is overriden. Subclasses
	 * must call super when overriding this method.
	 */
	...
	void recalculateRowHeaderHeight(GridItem item, int oldHeight, int newHeight) {
		...
		if (newHeight > itemHeight) {
			...
			itemHeight = computeItemHeight(getItem(0), sizingGC);

			for (int cnt = 0; cnt < getItemCount(); cnt++) {
				getItem(cnt).setHeight(itemHeight);
			}
	...
	void recalculateRowHeaderWidth(GridItem item, int oldWidth, int newWidth) {
		...
		if (newWidth > rowHeaderWidth) {
			...
			// if the changed width is smaller, and the previous width of that
			// rows header was equal
			// to the current row header width then its possible that we may
			// need to make the new
			// row header width smaller, but to do that we need to ask all the
			// rows all over again
			for (Iterator iter = itemIterator(); iter.hasNext();) {
				GridItem iterItem = (GridItem) iter.next();
				newWidth = Math.max(newWidth, rowHeaderRenderer.computeSize(
						sizingGC, SWT.DEFAULT, SWT.DEFAULT, iterItem).x);
			}
	...
	/**
	 * Sets the row header width to the specified value. This automatically
	 * disables the auto width feature of the grid.
	 * 
	 * @param width
	...
	private void initAccessible() {
		...
		accessible.addAccessibleListener(new AccessibleAdapter() {
			...
			public void getDescription(AccessibleEvent e) {
				...
				if (childID >= 0 && childID < getItemCount()) {
					String descrption = "";
					for (int i = 0; i < columns.size(); i++) {
						if (i != 0) {
							descrption += columns.get(i).getText() + " : ";
							descrption += getItem(childID).getText(i) + " ";
						}
					}
					e.result = descrption;
				}
			...
			public void getName(AccessibleEvent e) {
				...
				if (childID >= 0 && childID < getItemCount()) {
					// Name of the items
					e.result = getItem(childID).getText();
				} else if (childID >= getItemCount()
						&& childID < getItemCount() + columns.size()) {
					// Name of the column headers
					e.result = columns.get(childID - getItemCount()).getText();
				} else if (childID >= getItemCount() + columns.size()
						&& childID < getItemCount() + columns.size()
								+ columnGroups.length) {
					// Name of the column group headers
					e.result = columnGroups[childID - getItemCount()
							- columns.size()].getText();
				} else if (childID >= getItemCount() + columns.size()
						+ columnGroups.length
						&& childID < getItemCount() + columns.size()
								+ columnGroups.length + columnGroups.length) {
		...
		accessible.addAccessibleControlListener(new AccessibleControlAdapter() {
			...
			public void getChildAtPoint(AccessibleControlEvent e) {
				...
				if (item != null) {
					...
					if (column != null) {
						...
						GridColumnGroup columnGroup = overColumnGroupHeader(
			...
			public void getLocation(AccessibleControlEvent e) {
				...
				if (childID >= 0 && childID < getItemCount()) {
					...
					GridColumn column = getColumn(childID - getItemCount());
					if (column != null) {
					...
					GridColumnGroup columnGroup = getColumnGroup(childID
							- getItemCount() - columns.size());
					if (columnGroup != null) {
						location.y = 0;
						location.height = groupHeaderHeight;
						location.x = getColumnHeaderXPosition(columnGroup
					...
					GridColumnGroup columnGroup = getColumnGroup(childID
							- getItemCount() - columns.size()
							- columnGroups.length);
					location = ((DefaultColumnGroupHeaderRenderer) columnGroup
			...
			public void getState(AccessibleControlEvent e) {
				...
				if (childID >= 0 && childID < getItemCount()) {
					...
					if (selectedItems.contains(getItem(childID))) {
						...
						if (getDisplay().getActiveShell() == getParent()
								.getShell()) {
		...
		addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				if (selectedItems.size() > 0) {
					accessible.setFocus(getIndexOfItem(selectedItems
		...
		addTreeListener(new TreeListener() {
			...
			public void treeCollapsed(TreeEvent e) {
				if (getFocusItem() != null) {
					accessible.setFocus(getIndexOfItem(getFocusItem()));
				}
			}
	...
	/**
	 * Determines if the mouse is hovering on the selection drag area and
	 * changes the pointer and sets field appropriately.
	 * <p>
	 * Note: The 'selection drag area' is that part of the selection, on which a
	 * drag event can be initiated. This is either the border of the selection
	 * (i.e. a cell border between a slected and a non-selected cell) or the
	 * complete selection (i.e. anywhere on a selected cell). What area serves
	 * as drag area is determined by {@link #setDragOnFullSelection(boolean)}.
	 * 
	 * @param x
	 * @param y
	 * @return
	...
	private boolean handleHoverOnSelectionDragArea(int x, int y) {
		...
		if ((!rowHeaderVisible || x > rowHeaderWidth
				- SELECTION_DRAG_BORDER_THRESHOLD)
				&& (!columnHeadersVisible || y > headerHeight
						- SELECTION_DRAG_BORDER_THRESHOLD)) {
			// not on a header
			...
			//
			// over = (neSel || nwSel || swSel || seSel) && (!neSel || !nwSel ||
			// !swSel || !seSel);
			// // inSelection = neSel ? neP : nwSel ? nwP : swSel ? swP : seSel
			// ? seP : null;
			// }
			// else
			// {
	 ...
	 * @param column
	 *            the column of the cell. Null will make the insertion mark span
	 *            all columns.
	 * @param before
	 ...
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT - if the item or column has
	 *                been disposed</li>
	 *                </ul>
	 * @exception SWTException
	 *                <ul>
	...
	/**
	 * A helper method for
	 * {@link GridDropTargetEffect#dragOver(DropTargetEvent)}.
	 * 
	 * @param point
	 * @return true if point is near the top or bottom border of the visible
	 *         grid area
	 */
	boolean isInDragScrollArea(Point point) {
		...
		Rectangle top = new Rectangle(rhw, chh, getClientArea().width - rhw,
				DRAG_SCROLL_AREA_HEIGHT);
		Rectangle bottom = new Rectangle(rhw, getClientArea().height
				- DRAG_SCROLL_AREA_HEIGHT, getClientArea().width - rhw,
				DRAG_SCROLL_AREA_HEIGHT);
	 ...
	 * @param allChildren
	 *            <code>true</code> if all child items of the range of items
	 *            should be cleared recursively, and <code>false</code>
	 *            otherwise
	 * 
	 * @exception IllegalArgumentException
	 ...
	 *            the array of indices of the items
	 * @param allChildren
	 *            <code>true</code> if all child items of the indexed items
	 *            should be cleared recursively, and <code>false</code>
	 *            otherwise
	 * 
	 * @exception IllegalArgumentException
	...
	public void clear(int[] indices, boolean allChildren) {
		...
		for (int i = 0; i < indices.length; i++) {
			GridItem item = getItem(indices[i]);
			item.clear(allChildren);
		}
		redraw();
	}
	...
	public void clearAll(boolean allChildren) {
		checkWidget();
		if (getItemCount() > 0) {
			clear(0, getItemCount() - 1, allChildren);
		}
	}
		...
		if (topIndex <= bottomIndex) {
			if (getItemCount() > 0) {
				...
				for (int i = topIndex; i <= bottomIndex; i++) {
					range.items[i - topIndex] = getItem(i);
				}
			}
		}
	...
	/**
	 * Sets the value of the auto-height feature. When enabled, this feature
	 * resizes the height of rows to reflect the content of cells with
	 * word-wrapping enabled. Cell word-wrapping is enabled via the
	 * GridColumn.setWordWrap(boolean) method. If column headers have
	 * word-wrapping enabled, this feature will also resize the height of the
	 * column headers as necessary.
	 * 
	 * @param enabled
	...
	public void setAutoHeight(boolean enabled) {
		...
		checkWidget();
		autoHeight = enabled;
		setRowsResizeable(false); // turn of resizing of row height since it
									// conflicts with this property
		redraw();
	}

	/**
	 * Returns the value of the auto-height feature, which resizes row heights
	 * and column header heights based on word-wrapped content.
	 * 
	...
	/**
	 * Sets the value of the auto-width feature. When enabled, this feature
	 * resizes the width of the row headers to reflect the content of row
	 * headers.
	 * 
	 * @param enabled
	 *            Set to true to enable this feature, false (default) otherwise.
	...
	/**
	 * Returns the value of the auto-height feature, which resizes row header
	...
	/**
	 * Sets the value of the word-wrap feature for row headers. When enabled,
	 * this feature will word-wrap the contents of row headers.
	 * 
	 * @param enabled
	 *            Set to true to enable this feature, false (default) otherwise.
	...
	/**
	 * Returns the value of the row header word-wrap feature, which word-wraps
	 * the content of row headers.
	 * 
	 * @return Returns whether or not the row header word-wrap feature is
	 *         enabled.
